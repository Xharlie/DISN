...
import tf_util
import posenet
# import models.pointnet.model_fc_upconv as pointnet
...
def get_model(ref_dict, num_point, is_training, bn=False, bn_decay=None, img_size = (137,137), wd=1e-5):
    ...
    sample_img_points, gt_xy = get_img_points(ref_sample_pc, trans_mat)
    ...
    pred_sample_img_points, pred_xy = get_img_points(ref_sample_pc, pred_trans_mat)
...
def get_loss(end_points, sdf_weight=10., regularization=True, FLAGS=None):
    ...
    rot2d_dist = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(sample_img_points - pred_sample_img_points), axis = -1)))
    rot3d_dist = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(sub_3d), axis = -1)))
