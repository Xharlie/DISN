...
mp_distance_module=tf.load_op_library(os.path.join(BASE_DIR, 'tf_mpdistance_so.so'))

def mp_distance(xyz1,xyz2):
    '''
...
output: idx2:  (batch_size,#point_2)   nearest neighbor from second to first
    '''
    return mp_distance_module.mp_distance(xyz1,xyz2)
#@tf.RegisterShape('MpDistance')
#def _mp_distance_shape(op):
...
@ops.RegisterGradient('MpDistance')
def _mp_distance_grad(op,grad_dist1,grad_idx1,grad_dist2,grad_idx2):
    xyz1=op.inputs[0]
    xyz2=op.inputs[1]
    idx1=op.outputs[1]
    idx2=op.outputs[3]
    return mp_distance_module.mp_distance_grad(xyz1,xyz2,grad_dist1,idx1,grad_dist2,idx2)
...
if __name__=='__main__':
    ...
    with tf.Session('') as sess:
        ...
        if True:
            ...
            reta,retb,retc,retd=mp_distance(inp1,inp2)
